...
### Upload files
#compiled stats
stats <- read.csv("A2_compiled_stats.csv", header = T)
#no overhangs coverage
coverage <- read.csv("A2_coverage_no_overhangs.csv",header = T, sep = "\t") 
...
write.csv(all_coverage, "A2_cum_cov_per_site.csv")
...
write.csv(file="A2_counts_coverage.csv", count_coverage, row.names = FALSE)
