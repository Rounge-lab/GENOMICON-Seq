...
key_table<-read.csv("sample_key_index.csv", sep = "\t")
...
### Now load the variants, for each sample, extract the true false, and the true positives
all_variants<-read.csv("A2_all_variants.csv")


## Try to filter the true positive 

# Step 1: Ensure sample_id2 in all_variants is treated as a factor
...
sample_indexes_table<-key_table_short %>% 
  select(original_sample_index,number_copies,fragment_fraction) %>% 
...
true_and_false$true_false <- factor(true_and_false$true_false, levels = c("false", "true"))
true_and_false$original_sample_index <- factor(true_and_false$original_sample_index,
                                              levels = sort(unique(as.numeric(true_and_false$original_sample_index))))

false_color <- "#fa7f5e" # Placeholder color for 'Noise'
true_color <- "#1fa287"  # Placeholder color for 'OM'
...
gg <- ggplot(true_and_false, aes(x = original_sample_index, y = minor_base_count, fill = true_false)) +
  ...
  scale_y_log10(limits = c(1, 10000)) +  # Check these limits based on your data
...
ggsave("A2_mutations_vs_noise_split_violin_all_mutation.jpg", gg, width = 5.625, height = 3, units = "in")
...
write.csv(summary_table,"A2_summary_read_counts_counts_all_mutations.csv",)
