...
# Convert mutation_rate and polymerase_error_rate to factors if they are not already
summary_all_mutations$mutation_rate <- as.factor(summary_all_mutations$mutation_rate)
summary_all_mutations$polymerase_error_rate <- as.factor(summary_all_mutations$polymerase_error_rate)


# Calculate F1 Score and find the optimal cutoff
summary_all_mutations <- summary_all_mutations %>%
  group_by(original_sample_index) %>%
  mutate(F1 = 2 * (mean_precision * mean_recall) / (mean_precision + mean_recall),  # Calculate F1 Score
         optimal = F1 == max(F1, na.rm = TRUE)) %>%  # Find max F1 Score
  ungroup()
...
p <- ggplot(data = summary_all_mutations, aes(x = read_cutoff)) + # Lines for mean_precision
  ...
  # Faceting
  facet_wrap(~ original_sample_index, ncol = 4, nrow = 1, scales = "free_x") +
  
  # Labels and themes
  labs(title = "Precision and Recall by Read Cutoff",
       x = "Read Cutoff",
       color = "Metric") +
  theme_minimal() +
...
ggsave("A3_precision_vs_recall_all_mut.jpg", p, height = 6, width = 6, units = "in")


### now lets make the APOBEC

### make sample_Table to store the results

results_table_AP_all_samples_all_cutoffs<-key_table_short %>%filter(!original_sample_index %in% c(13,14,15)) %>%  select(-sample_table_id,-pcr_table_id,-number_reads)
...
# Convert mutation_rate and polymerase_error_rate to factors 
summary_APOBEC_mutations$mutation_rate <- as.factor(summary_APOBEC_mutations$mutation_rate)
summary_APOBEC_mutations$polymerase_error_rate <- as.factor(summary_APOBEC_mutations$polymerase_error_rate)
...
p <- ggplot(data = summary_APOBEC_mutations, aes(x = read_cutoff)) +
  ...
  # Faceting
  facet_wrap(~ original_sample_index, ncol = 4, nrow = 3, scales = "free_x") +
...
ggsave("A3_precision_vs_recall_APOBEC_mut.jpg", p, height = 6, width = 6, units = "in")



### save tables 

write.csv(x = results_table_all_samples_all_cutoffs, "A3_all_mutations_FP_TP_FN_per_tech_rep.csv", row.names = F)
write.csv(x = summary_all_mutations, "A3_all_mutations_FP_TP_FN_summary_per_sample.csv", row.names = F)
write.csv(x = results_table_AP_all_samples_all_cutoffs, "A3_APOBEC_FP_TP_FN_per_tech_rep.csv", row.names = F)
write.csv(x = summary_APOBEC_mutations, "A3_APOBEC_FP_TP_FN_summary_per_sample.csv", row.names = F)
