...
write.csv(summary_table, "A1_unique_mutations_overview.csv")
...
# Creating phantom data for missing 'true' entries in samples 1, 2, 3 and 4 (where the mutation rate is 0, no true mutations)
phantom_data <- data.frame(
  original_sample_index = c(1, 2, 3, 4), 
  tech_rep = c(1, 1, 1, 1),  
  position = c(1, 1, 1, 1),  
  minor_base_count = c(1, 1, 1, 1),  
  true_false = c("true", "true", "true", "true")
)


# Combine the real data with phantom data
combined_data <- rbind(true_and_false, phantom_data)
combined_data$true_false <- factor(combined_data$true_false, levels = c("false", "true"))
combined_data$original_sample_index <- factor(combined_data$original_sample_index,
                                              levels = sort(unique(as.numeric(combined_data$original_sample_index))))

false_color <- "#fa7f5e" 
true_color <- "#1fa287"  

gg <- ggplot(combined_data, aes(x = original_sample_index, y = minor_base_count, fill = true_false)) +
...
ggsave("A1_mutations_vs_noise_split_violin_all_mutation.jpg", gg, width = 10, height = 3, units = "in")
...
write.csv(summary_table,"A1_summary_read_counts_counts_all_mutations.csv",)
