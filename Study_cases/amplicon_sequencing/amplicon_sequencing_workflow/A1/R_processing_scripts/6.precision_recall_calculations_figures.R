...
key_table<-read.csv("A1_sample_key_index.csv", sep = "\t")

sample_tables<-read.csv("A1_sample_tables.csv")
...
all_variants<-read.csv("A1_all_variants.csv")
...
## Remove samples with 0 mutations
summary_all_mutations<-summary_all_mutations %>% filter(!original_sample_index %in% c(1,2,3,4))
...
ggsave("A1_precision_vs_recall_all_mut.jpg", p, height = 6, width = 6, units = "in")
...
ggsave("A1_precision_vs_recall_APOBEC_mut.jpg", p, height = 6, width = 6, units = "in")



### save tables 

write.csv(x = results_table_all_samples_all_cutoffs, "A1_all_mutations_FP_TP_FN_per_tech_rep.csv", row.names = F)
write.csv(x = summary_all_mutations, "A1_all_mutations_FP_TP_FN_summary_per_sample.csv", row.names = F)
write.csv(x = results_table_AP_all_samples_all_cutoffs, "A1_APOBEC_FP_TP_FN_per_tech_rep.csv", row.names = F)
write.csv(x = summary_APOBEC_mutations, "A1_APOBEC_FP_TP_FN_summary_per_sample.csv", row.names = F)
