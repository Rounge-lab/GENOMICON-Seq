...
### Upload files
#compiled stats
stats <- read.csv("A1_compiled_stats.csv", header = T)
#no overhangs coverage
coverage <- read.csv("A1_coverage_no_overhangs.csv",header = T, sep = "\t") 
...
write.csv(all_coverage, "R_res/Run_6_cum_cov_per_site.csv")
...
write.csv(file="A1_counts_coverage.csv", count_coverage, row.names = FALSE)
