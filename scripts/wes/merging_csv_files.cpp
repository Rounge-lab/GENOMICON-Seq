...
void concatenate_csv(const std::string& directory, const std::string& output_file) {
    ...
    out << "Names;Mutation_Positions;Copies;genome_coordinates\n";
    ...
    if ((dir = opendir(directory.c_str())) != nullptr) {
        while ((ent = readdir(dir)) != nullptr) {
            ...
            if (filename.find("_temp.csv") != std::string::npos) {
...
double calculate_sum(const std::string& input_file) {
    ...
    while (std::getline(in, line)) {
        ...
        if (tokens.size() >= 3) {
            try {
                sum += std::stod(tokens[2]);
            } catch (const std::invalid_argument& e) {
                std::cerr << "Invalid number encountered in column 3: " << tokens[2] << std::endl;
            }
        }
    }
...
void process_csv_to_txt(const std::string& input_file, const std::string& output_file, double sum) {
    ...
    while (std::getline(in, line)) {
        auto tokens = split(line, ';');
        if (tokens.size() >= 3) {
            std::string combined = tokens[0] + "_" + tokens[1];
            std::replace(combined.begin(), combined.end(), ' ', '_');
            std::replace(combined.begin(), combined.end(), ',', '_');

            double normalizedValue = std::stod(tokens[2]) / sum; 
