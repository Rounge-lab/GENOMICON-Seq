...
def process_files(mutation_occurrence_file, detailed_mutation_file, output_file):
    try:
        # Load and check the occurrence file
        df1 = pd.read_csv(mutation_occurrence_file)
        if df1.empty:
            print(f"Skipping {mutation_occurrence_file} as it is empty.")
            return

        # Load the detailed mutation file
        df2 = pd.read_csv(detailed_mutation_file, sep=',')  # Adjust delimiter if needed
        
        # Extract position and variant from the mutation string
       ...
       # Drop the 'pos_var' column if it exists
        ...
        if not result.empty:
            ...
            # Optionally remove input files after processing
            os.remove(mutation_occurrence_file)
            os.remove(detailed_mutation_file)
            print(f"Input files removed after successful processing.")
        else:
...
def main():
    parser = argparse.ArgumentParser(description="Process mutation files by merging.")
